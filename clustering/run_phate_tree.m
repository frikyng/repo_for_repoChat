...
N_Dim = 9; % can set to # of usable (non-discarded) ROIs in the object. Ideally, use a square number to have a nicer layout
...
%%
clearvars -except N_Dim conditions path source_signal type_of_trace analysis_mode obj signal_indices time_filter use_hd_data
...
for el = 1:numel(conditions)
    %% Get phate components
    [~, current_signal] = obj.get_dimensionality(source_signal, 'phate', [conditions{el},'_subtracted'], N_Dim, 't', []);
    Y_PHATE_3D{el} = obj.dimensionality.LoadingsPM;
    valid_ROIs = obj.dimensionality.valid_trace_idx;
    figure();imagesc(Y_PHATE_3D{el});colorbar;
